...
import logo from "@/assets/ciuAttendLogo.png";
...
export default function StudentDashboardPage() {
  ...
  const [lastCheckInStatus, setLastCheckInStatus] =
    useState<LastCheckInStatus | null>(null);
  ...
  useEffect(() => {
    if (!authLoading && user?.role === "student") {
      // Only poll if user is a student and auth is loaded
      fetchSessionStatus();
    ...
    if (
      ...
      const updateTimer = () => {
        ...
        if (new Date() < autoCloseDate) {
          setTimeRemaining(
            formatDistanceToNowStrict(autoCloseDate, { addSuffix: true })
          );
        } else {
  ...
  const handleCheckIn = async () => {
    if (
      ...
      toast({
        ...
        description:
          "Attendance is not currently open or user/device info is missing.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        toast({
          ...
          description:
            result.message ||
            `You've been marked present for session ${attendanceSession.sessionId}.`,
          variant: "default",
  ...
  const isCheckedInForCurrentSession =
    lastCheckInStatus?.success &&
    lastCheckInStatus.sessionId === attendanceSession?.sessionId;
  const canAttemptCheckIn =
    attendanceSession?.status === "open" && !isCheckedInForCurrentSession;
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-background to-secondary/10">
      <header className="p-4 md:p-6 flex justify-between items-center shadow-sm bg-card/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="flex items-center">
          <Image
            src={logo}
            className="w-10 h-10 md:w-16 md:h-16 mr-2 md:mr-3 text-primary"
            alt="Logo"
          />
            ...
            <CardContent>
              ...
              {deviceId && (
                <div className="mt-2 text-xs text-muted-foreground flex items-center">
                  <Smartphone className="w-3 h-3 mr-1" /> Device ID:{" "}
                  <span className="font-mono ml-1 truncate" title={deviceId}>
          ...
          <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 md:col-span-2">
            <CardHeader className="pb-2">
              <CardTitle className="text-lg font-medium flex items-center">
                ...
                {/* Using Wifi icon as proxy for BLE/Connectivity */}
                Attendance Check-in
              </CardTitle>
              {attendanceSession?.sessionId && (
                <CardDescription>
                  Session ID: {attendanceSession.sessionId}
                </CardDescription>
              )}
            </CardHeader>
            <CardContent>
                ...
                <div className="p-3 mb-4 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700 rounded-lg text-yellow-700 dark:text-yellow-300">
                  <div className="flex items-center">
                    ...
                    <p className="font-semibold">
                      No active attendance session. Please wait for the
                      instructor to start one.
                    </p>
                  </div>
                </div>
              )}
              {(attendanceSession?.status === "closed_manual" ||
                attendanceSession?.status === "closed_timeout") && (
                <div className="p-3 mb-4 bg-red-50 dark:bg-red-900/30 border border-red-300 dark:border-red-700 rounded-lg text-red-700 dark:text-red-300">
                  ...
                  {attendanceSession.endTime && (
                    <p className="text-xs text-muted-foreground mt-1">
                      Closed at:{" "}
                      {new Date(attendanceSession.endTime).toLocaleTimeString()}
                      {attendanceSession.status === "closed_timeout" &&
                        " (timed out)"}
                    </p>
                  )}
      ...
      <footer className="text-center p-4 text-xs text-muted-foreground border-t mt-auto">
        CIUAttend &copy; {new Date().getFullYear()} :: Student View
      </footer>
    </div>
  );
}
